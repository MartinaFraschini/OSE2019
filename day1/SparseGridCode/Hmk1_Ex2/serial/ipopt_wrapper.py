...
def EV_G(X, k_init, n_agents):
    ...
    f_prod=output_f(k_init, lab, theta_inf)
...
def EV_G_ITER(X, k_init, n_agents, curr_theta):
    N=len(X)
    M=3*n_agents+1  # number of constraints
    G=np.empty(M, float)
    ...
    f_prod=output_f(k_init, lab, curr_theta)
    Gamma_adjust=0.5*zeta*k_init*((inv/k_init - delta)**2.0)
    sectors_sum=cons + inv - delta*k_init - (f_prod - Gamma_adjust)
    G[3*n_agents]=np.sum(sectors_sum)
...
def EV_JAC_G_ITER(X, flag, k_init, n_agents, curr_theta):
    N=len(X)
    M=3*n_agents+1
    NZ=M*N
    A=np.empty(NZ, float)
    ...
    if (flag):
        ...
        # Finite Differences
        h=1e-4
        gx1=EV_G_ITER(X, k_init, n_agents, curr_theta)
        
        for ixM in range(M):
            for ixN in range(N):
                xAdj=np.copy(X)
                xAdj[ixN]=xAdj[ixN]+h
                gx2=EV_G_ITER(xAdj, k_init, n_agents, curr_theta)
                A[ixN + ixM*N]=(gx2[ixM] - gx1[ixM])/h
        return A    
